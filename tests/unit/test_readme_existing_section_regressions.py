...
def _assert_compatibility_claim_block(
    ...
    assert all(
        error == expected or "substantive generated bytes lack exact candidate authority" in error
        for error in decision.errors
    )
...
"""
    ...
    assert plan.claim_accountability is not None
    blocking_claims = [
        claim for claim in plan.claim_accountability.claims if not claim.currently_accountable
    ]
    blocking_prefix = f"claim accountability has {len(blocking_claims)} blocking claim(s):"
    assert decision.errors[-1].startswith(blocking_prefix)
    assert all(
        error.startswith("unauthorized protected-content loss:")
        or "substantive generated bytes lack exact candidate authority" in error
        or error.startswith(blocking_prefix)
        for error in decision.errors
    )
