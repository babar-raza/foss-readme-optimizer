...
regression-testing the RDM-030 fix on 2026-08-27:
test_verified_template_api_descriptions.py::
test_spreadsheet_io_and_encryption_functions_have_concrete_descriptions and
...
RDM-030 change (verified_template_api_text.py::role_sentence()'s family-
fallback hardening) fully reverted.

Run once from the repo root:
`.venv/Scripts/python scripts/retrofits/update_ver013_add_two_more_tests.py`
...
NEW_REQUIREMENT = (
    "Six failing unit tests found during unrelated regression testing on "
...
NEW_EVIDENCE_ADDENDUM = (
    " VER013-STALE-TESTS-003 (2026-08-27): the two api_descriptions/structural_lineage "
    "tests also found pre-existing via the same stash-isolation method, while regression-"
    "testing an unrelated verified_template_api_text.py::role_sentence() fix (RDM-030). "
    "Both reproduce identically with that change fully reverted."
)
...
def main() -> None:
    ...
    for index, line in enumerate(lines):
        ...
        if record.get("requirement_id") != "VER-013":
            continue
        if "VER013-STALE-TESTS-003" in record.get("acceptance_evidence", ""):
