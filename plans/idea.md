...
content. Additional-examples prose previews the named workflows a visitor can expand; it never exposes internal inventory, source-revision, syntax-check, static-API-check, or non-execution commentary. Every expanded workflow uses a meaningful task name rather than a duplicated generic heading with a numeric suffix. Every source fence carries its correct language identifier and visitor examples use normalized, language-valid spacing without repeated empty-line runs. Key-capability titles are natural action-led search phrases grounded in accepted product, platform, input-format, and output-format facts without keyword stuffing. Omit redundant “Other
