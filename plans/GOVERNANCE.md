...
| `data/` | Config-as-data JSON (registry, link database, and the pinned `data/imported/` aspose.org development-oracle corpus — committed, checksum-bound, development-only evidence per Decision 106, never a runtime dependency). Every file here has a section in `data/README.md` stating what it is, who/what produces it, and how it's refreshed. |
...
| `report/` | Findings written *for* an external audience this project doesn't control — a product team or agent that owns a blocked upstream repository — as opposed to `plans/investigations/`, which is this project's own internal audit trail. `report/findings/<family>/<platform>/<self-explanatory-name>.md`, one report per upstream defect investigation, self-explanatory-named per the naming rules above. Every report cites its independent evidence trail (the investigation, its evidence pointers, and any corroborating prior record) so its claims are reproducible without re-trusting the report's prose. |
