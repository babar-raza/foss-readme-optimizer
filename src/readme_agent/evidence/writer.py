...
def win_long_path(path: Path | str) -> str:
    ...
    find the path specified", which surfaces as a `FileNotFoundError` from
    `os.replace` naming a temp file that plainly exists, or as a `shutil.Error`
    from `shutil.copytree` for the same reason. Bounded review hit the first
    shape for real: a packet-cache entry is
    ...
    `aspose-3d-foss__Aspose.3D-FOSS-for-Python` -- so the failure tracked
    repository *name length* rather than anything about the repository. The
    superseded-evidence archive path hits the second shape the same way (see
    `local_poc_superseded.py`'s callers).
    ...
    Public: shared by any caller passing a path straight to a Windows file API
    (`os.replace`, `shutil.copytree`, `shutil.rmtree`, ...), not only this module.
    """
