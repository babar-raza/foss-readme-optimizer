...
def record_trusted_review_execution(
    ...
    *,
    ...
    if lifecycle.status in {"TRUSTED_REVIEW_REJECTED", "SYSTEM_FAILURE"}:
        if lifecycle.status == "TRUSTED_REVIEW_REJECTED" and (
            lifecycle.prompt_hash is None or lifecycle.prompt_hash == prompt_hash
        ):
            raise StateBackendError(
    ...
    if lifecycle.status == "TRUSTED_REPAIRING":
        lifecycle = transition_trusted_readme_poc_status(
            backend,
            org_repo,
            "TRUSTED_CANDIDATE_GENERATED",
            observed_by="trusted_review",
            reason="resume an interrupted trusted repair at candidate revalidation",
            evidence_refs=evidence_refs,
            source_revision=graph.source_revision,
            facts_hash=graph.canonical_hash(),
            candidate_hash=composition.candidate_sha256,
