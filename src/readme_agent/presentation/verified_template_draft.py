...
def source_tree_installation_fact_ids(facts: ProductFactsV2) -> list[str]:
    """Return the accepted fact IDs `source_tree_installation_text` actually cites.
    ...
    rejected the whole candidate with "substantive generated bytes lack exact
    candidate authority" against the fallback clone-and-import block -- real,
    verified content (the repository URL comes directly from `product.identity`)
    that nonetheless received no `CandidateContentProvenanceV1` binding, because
    the caller in `verified_template_provenance.py` only ever unioned in the
    installation slot's generic `content.fact_ids`, which is empty whenever this
    fallback is the *only* installation content (no other accepted installation
    fact exists to populate it). Mirrors this function's own fact selection
    exactly -- `product.identity` always, `api.public_surface` only when its
    `canonical_import` made it into the rendered text -- gated by the same
    verification-state and no-unresolved-conflict bar every other binding in
    this pipeline already requires.
    """

    text = source_tree_installation_text(facts)
    if text is None:
        return []
    fact_ids: list[str] = []
    identity = facts.selected_fact("product.identity")
    if (
        ...
        fact_ids.append(identity.fact_id)
    if "imports as" in text:
        try:
            api = facts.selected_fact("api.public_surface")
        except KeyError:
            api = None
        if (
            api is not None
            and api.verification_state in {"verified", "policy_approved"}
            and not api.has_unresolved_conflict
        ):
            fact_ids.append(api.fact_id)
