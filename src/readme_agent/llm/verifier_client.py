...
class LiveForcedToolClient:
    ...
    def call(self, messages: list[dict], tool_schema: dict) -> ForcedToolResult:
        ...
        for attempt in range(_MAX_RETRIES + 1):
            try:
                resp = self._post_once(messages, tool_schema)
            except (requests.ConnectionError, requests.Timeout) as exc:
                last_error = f"network error: {exc}"
                if attempt < _MAX_RETRIES:
                    time.sleep(_BACKOFF_SECONDS[attempt])
                    continue
                raise LLMError(f"forced tool call failed after retries: {last_error}") from exc
            ...
            if resp.status_code in _RETRYABLE_STATUS:
                ...
                if attempt < _MAX_RETRIES:
                    time.sleep(_BACKOFF_SECONDS[attempt])
                    continue
                raise LLMError(f"forced tool call failed after retries: {last_error}")
        ...
        raise LLMError(f"forced tool call failed after retries: {last_error}")  # pragma: no cover
