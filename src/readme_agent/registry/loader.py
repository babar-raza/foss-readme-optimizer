...
from readme_agent.registry.naming import validate_managed_repository_coordinates
...
def load_products(products_path: Path = PRODUCTS_PATH) -> tuple[ProductEntry, ...]:
    ...
    for i, item in enumerate(raw):
        try:
            entry = ProductEntry.model_validate(item)
        ...
        try:
            ...
            raise ConfigError(f"{products_path}[{i}] is ineligible: {exc}") from exc
