...
| `ArbitraryProfile` | `ArbitraryProfile.get_bounding_box() -> BoundingBox` | Supports retrieving bounding box through `ArbitraryProfile`. Inherited from `Entity`. |
...
| `BooleanOperator` | `BooleanOperator.get_bounding_box() -> BoundingBox` | Supports retrieving bounding box through `BooleanOperator`. Inherited from `Entity`. |
...
| `CShape` | `CShape.get_bounding_box() -> BoundingBox` | Supports retrieving bounding box through `CShape`. Inherited from `Entity`. |
...
| `CenterLineProfile` | `CenterLineProfile.get_bounding_box() -> BoundingBox` | Supports retrieving bounding box through `CenterLineProfile`. Inherited from `Entity`. |
...
| `CircleShape` | `CircleShape.get_bounding_box() -> BoundingBox` | Supports retrieving bounding box through `CircleShape`. Inherited from `Entity`. |
...
| `CompositeCurve` | `CompositeCurve.get_bounding_box() -> BoundingBox` | Supports retrieving bounding box through `CompositeCurve`. Inherited from `Entity`. |
...
| `Curve` | `Curve.get_bounding_box() -> BoundingBox` | Supports retrieving bounding box through `Curve`. Inherited from `Entity`. |
...
| `EllipseShape` | `EllipseShape.get_bounding_box() -> BoundingBox` | Supports retrieving bounding box through `EllipseShape`. Inherited from `Entity`. |
...
| `Entity` | `Entity.get_bounding_box() -> BoundingBox` | Supports retrieving bounding box through `Entity`. |
...
| `EntityRenderer` | `EntityRenderer.render(entity, context)` | Calls the `render` operation on `EntityRenderer`. |
| `FMatrix4` | `FMatrix4.inverse() -> 'FMatrix4'` | Calls the `inverse` operation on `FMatrix4`. |
...
| `FontFile` | `FontFile.from_file(file_name) -> 'FontFile'` | Supports loading content from file through `FontFile`. |
| `FontFile` | `FontFile.name: str` | Gets the `name` property on `FontFile`. Inherited from `A3DObject`. |
| `FormatDetector` | `FormatDetector.detect(stream, file_name) -> 'FileFormat'` | Supports detecting changes through `FormatDetector`. |
...
| `HShape` | `HShape.get_bounding_box() -> BoundingBox` | Supports retrieving bounding box through `HShape`. Inherited from `Entity`. |
...
| `HalfSpace` | `HalfSpace.get_bounding_box()` | Supports retrieving bounding box through `HalfSpace`. |
...
| `HollowCircleShape` | `HollowCircleShape.get_bounding_box() -> BoundingBox` | Supports retrieving bounding box through `HollowCircleShape`. Inherited from `Entity`. |
...
| `HollowRectangleShape` | `HollowRectangleShape.get_bounding_box() -> BoundingBox` | Supports retrieving bounding box through `HollowRectangleShape`. Inherited from `Entity`. |
...
| `IDescriptorSet` | `IDescriptorSet.set_texture(binding, texture)` | Supports setting texture through `IDescriptorSet`. |
| `IMeshConvertible` | `IMeshConvertible.to_mesh() -> 'Mesh'` | Supports converting content to mesh through `IMeshConvertible`. |
...
| `IOrientable` | `IOrientable.get_bounding_box() -> BoundingBox` | Supports retrieving bounding box through `IOrientable`. Inherited from `Entity`. |
...
| `LShape` | `LShape.get_bounding_box() -> BoundingBox` | Supports retrieving bounding box through `LShape`. Inherited from `Entity`. |
...
| `LambertMaterial` | `LambertMaterial.get_texture(slot_name)` | Supports retrieving texture through `LambertMaterial`. Inherited from `Material`. |
| `LambertMaterial` | `LambertMaterial.name: str` | Gets the `name` property on `LambertMaterial`. Inherited from `A3DObject`. |
| `LambertMaterial` | `LambertMaterial.set_texture(slot_name, texture)` | Supports setting texture through `LambertMaterial`. Inherited from `Material`. |
...
| `Line` | `Line.get_bounding_box() -> BoundingBox` | Supports retrieving bounding box through `Line`. Inherited from `Entity`. |
...
| `LinearExtrusion` | `LinearExtrusion.get_bounding_box() -> BoundingBox` | Supports retrieving bounding box through `LinearExtrusion`. Inherited from `Entity`. |
...
| `LinearExtrusion` | `LinearExtrusion.to_mesh() -> 'Mesh'` | Supports converting content to mesh through `LinearExtrusion`. |
| `Material` | `Material.get_texture(slot_name)` | Supports retrieving texture through `Material`. |
| `Material` | `Material.name: str` | Gets the `name` property on `Material`. Inherited from `A3DObject`. |
| `Material` | `Material.set_texture(slot_name, texture)` | Supports setting texture through `Material`. |
...
| `Mesh` | `Mesh.difference(a, b) -> 'Mesh'` | Calls the `difference` operation on `Mesh`. |
| `Mesh` | `Mesh.do_boolean(op, a, transform_a, b, transform_b) -> 'Mesh'` | Supports doing boolean through `Mesh`. |
...
| `Mesh` | `Mesh.intersect(a, b) -> 'Mesh'` | Calls the `intersect` operation on `Mesh`. |
...
| `Mesh` | `Mesh.union(a, b) -> 'Mesh'` | Calls the `union` operation on `Mesh`. |
...
| `MirroredProfile` | `MirroredProfile.get_bounding_box() -> BoundingBox` | Supports retrieving bounding box through `MirroredProfile`. Inherited from `Entity`. |
...
| `NurbsCurve` | `NurbsCurve.evaluate(steps)` | Calls the `evaluate` operation on `NurbsCurve`. |
| `NurbsCurve` | `NurbsCurve.excluded: bool` | Gets the `excluded` property on `NurbsCurve`. Inherited from `Entity`. |
| `NurbsCurve` | `NurbsCurve.get_bounding_box() -> BoundingBox` | Supports retrieving bounding box through `NurbsCurve`. Inherited from `Entity`. |
...
| `NurbsSurface` | `NurbsSurface.to_mesh()` | Supports converting content to mesh through `NurbsSurface`. |
...
| `ParameterizedProfile` | `ParameterizedProfile.get_bounding_box() -> BoundingBox` | Supports retrieving bounding box through `ParameterizedProfile`. Inherited from `Entity`. |
...
| `PbrMaterial` | `PbrMaterial.get_texture(slot_name)` | Supports retrieving texture through `PbrMaterial`. Inherited from `Material`. |
...
| `PbrMaterial` | `PbrMaterial.set_texture(slot_name, texture)` | Supports setting texture through `PbrMaterial`. Inherited from `Material`. |
...
| `PbrSpecularMaterial` | `PbrSpecularMaterial.get_texture(slot_name) -> 'TextureBase'` | Supports retrieving texture through `PbrSpecularMaterial`. |
...
| `PbrSpecularMaterial` | `PbrSpecularMaterial.set_texture(slot_name, texture) -> None` | Supports setting texture through `PbrSpecularMaterial`. |
...
| `PhongMaterial` | `PhongMaterial.get_texture(slot_name)` | Supports retrieving texture through `PhongMaterial`. Inherited from `Material`. |
...
| `PhongMaterial` | `PhongMaterial.set_texture(slot_name, texture)` | Supports setting texture through `PhongMaterial`. Inherited from `Material`. |
...
| `Primitive` | `Primitive.to_mesh()` | Supports converting content to mesh through `Primitive`. |
...
| `Profile` | `Profile.get_bounding_box() -> BoundingBox` | Supports retrieving bounding box through `Profile`. Inherited from `Entity`. |
...
| `Quaternion` | `Quaternion.get_keyframe_sequence(anim, create)` | Supports retrieving keyframe sequence through `Quaternion`. |
...
| `RectangleShape` | `RectangleShape.get_bounding_box() -> BoundingBox` | Supports retrieving bounding box through `RectangleShape`. Inherited from `Entity`. |
...
| `Renderer` | `Renderer.render(render_target) -> None` | Calls the `render` operation on `Renderer`. |
| `RevolvedAreaSolid` | `RevolvedAreaSolid.excluded: bool` | Gets the `excluded` property on `RevolvedAreaSolid`. Inherited from `Entity`. |
| `RevolvedAreaSolid` | `RevolvedAreaSolid.get_bounding_box() -> BoundingBox` | Supports retrieving bounding box through `RevolvedAreaSolid`. Inherited from `Entity`. |
...
| `Scene` | `Scene.render(camera, file_name_or_bitmap, size=None, format=None, options=None)` | Calls the `render` operation on `Scene`. |
...
| `ShaderMaterial` | `ShaderMaterial.get_texture(slot_name) -> 'TextureBase'` | Supports retrieving texture through `ShaderMaterial`. |
| `ShaderMaterial` | `ShaderMaterial.name: str` | Gets the `name` property on `ShaderMaterial`. |
| `ShaderMaterial` | `ShaderMaterial.set_texture(slot_name, texture) -> None` | Supports setting texture through `ShaderMaterial`. |
...
| `Shape` | `Shape.get_bounding_box() -> BoundingBox` | Supports retrieving bounding box through `Shape`. Inherited from `Entity`. |
...
| `Skeleton` | `Skeleton.get_bounding_box() -> BoundingBox` | Supports retrieving bounding box through `Skeleton`. Inherited from `Entity`. |
...
| `SweptAreaSolid` | `SweptAreaSolid.get_bounding_box() -> BoundingBox` | Supports retrieving bounding box through `SweptAreaSolid`. Inherited from `Entity`. |
...
| `TShape` | `TShape.get_bounding_box() -> BoundingBox` | Supports retrieving bounding box through `TShape`. Inherited from `Entity`. |
...
| `Text` | `Text.get_bounding_box() -> BoundingBox` | Supports retrieving bounding box through `Text`. Inherited from `Entity`. |
...
| `Texture` | `Texture.set_rotation(u, v) -> None` | Supports setting rotation through `Texture`. |
| `Texture` | `Texture.set_scale(u, v) -> None` | Supports setting scale through `Texture`. |
| `Texture` | `Texture.set_translation(u, v) -> None` | Supports setting translation through `Texture`. |
...
| `TransformBuilder` | `TransformBuilder.rotate(q) -> 'TransformBuilder'` | Calls the `rotate` operation on `TransformBuilder`. |
| `TransformBuilder` | `TransformBuilder.scale(s) -> 'TransformBuilder'` | Calls the `scale` operation on `TransformBuilder`. |
| `TransformBuilder` | `TransformBuilder.translate(tx, ty, tz) -> 'TransformBuilder'` | Calls the `translate` operation on `TransformBuilder`. |
| `TransformedCurve` | `TransformedCurve.excluded: bool` | Gets the `excluded` property on `TransformedCurve`. Inherited from `Entity`. |
| `TransformedCurve` | `TransformedCurve.get_bounding_box() -> BoundingBox` | Supports retrieving bounding box through `TransformedCurve`. Inherited from `Entity`. |
...
| `TrapeziumShape` | `TrapeziumShape.get_bounding_box() -> BoundingBox` | Supports retrieving bounding box through `TrapeziumShape`. Inherited from `Entity`. |
...
| `TriMesh` | `TriMesh.get_bounding_box() -> BoundingBox` | Supports retrieving bounding box through `TriMesh`. Inherited from `Entity`. |
...
| `TrimmedCurve` | `TrimmedCurve.get_bounding_box() -> BoundingBox` | Supports retrieving bounding box through `TrimmedCurve`. Inherited from `Entity`. |
...
| `UShape` | `UShape.get_bounding_box() -> BoundingBox` | Supports retrieving bounding box through `UShape`. Inherited from `Entity`. |
...
| `VertexDeclaration` | `VertexDeclaration.clear()` | Supports clearing content through `VertexDeclaration`. |
...
| `VertexElementEdgeCrease` | `VertexElementEdgeCrease.clear()` | Supports clearing content through `VertexElementEdgeCrease`. |
...
| `VertexElementHole` | `VertexElementHole.clear()` | Supports clearing content through `VertexElementHole`. |
...
| `VertexElementMaterial` | `VertexElementMaterial.clear()` | Supports clearing content through `VertexElementMaterial`. |
...
| `VertexElementPolygonGroup` | `VertexElementPolygonGroup.clear()` | Supports clearing content through `VertexElementPolygonGroup`. |
...
| `VertexElementSpecular` | `VertexElementSpecular.clear()` | Supports clearing content through `VertexElementSpecular`. |
...
| `VertexElementUserData` | `VertexElementUserData.clear()` | Supports clearing content through `VertexElementUserData`. |
...
| `VertexElementVector4` | `VertexElementVector4.clear()` | Supports clearing content through `VertexElementVector4`. |
...
| `VertexElementVertexCrease` | `VertexElementVertexCrease.clear()` | Supports clearing content through `VertexElementVertexCrease`. |
...
| `VertexElementVisibility` | `VertexElementVisibility.clear()` | Supports clearing content through `VertexElementVisibility`. |
...
| `VertexElementWeight` | `VertexElementWeight.clear()` | Supports clearing content through `VertexElementWeight`. |
...
| `ZShape` | `ZShape.get_bounding_box() -> BoundingBox` | Supports retrieving bounding box through `ZShape`. Inherited from `Entity`. |
