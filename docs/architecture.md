...
|---|---|
| `registry/` | `data/products.json` + `config/policies/*.yml` loading and the execution allow-list gate; `priority.py` validates `data/platform_priorities.json` and gives canonical portfolio runs Python -> .NET -> Java -> C++ -> TypeScript -> Rust -> Go order while retaining stable within-ecosystem registry order; `surface_ownership.py` materializes the five control classes and their allowed operations, permissions, and rollback; `discovery.py` owns the read-only GitHub client, naming hint, and compatibility discovery view; `discovery_models.py` defines typed source/observation/inventory contracts; `discovery_inventory.py` retains every matched, ambiguous, and unmatched repository and explicit source failure; `reconciliation.py` and `reconciliation_models.py` migrate and reconcile entries by stable provider repository identity while preserving policy-owned fields and retaining variants; `intake.py` deterministically classifies a source snapshot as preserve-first, ordinary full-pipeline, narrowly blocked, not applicable, or failed without granting approval or effect permission; `self_heal.py` applies the discovery ledger before the public CLI execution allow-list gate and cannot report `NO_DRIFT` from an incomplete inventory (`CORE-034`, `L8-035`, `L8-036`, `L8-037`) |
