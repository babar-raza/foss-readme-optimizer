...
| `capabilities/` | Wave 2 (decision #27): `schema.py` (`CapabilityManifest`/`CapabilityGap`), `registry.py` (dispatch table, mirrors `ecosystems/registry.py`'s pattern), `dispatcher.py` (permission-aware tool-call dispatch), one file per capability (`inspect_repository.py`, `detect_readme_gaps.py`, `check_install_path.py`, `profile_repository.py`, `get_product_facts.py`, `classify_upstream_change.py`, `render_readme_candidate.py`, `audit_github_generated_surfaces.py`, `audit_package_release_surfaces.py`, `propose_metadata_changes.py`, `audit_community_files.py`, `prepare_visual_asset.py`) wrapping already-proven `orchestrator`/`readme`/`ecosystems`/`profile`/`github_api`/`inspection.file_inventory` functions — all `read_only_local`/`read_only_network`. `commit_readme_write.py` (Wave 7g, decision #41): this project's first and only registered mutating capability (`local_write`, scoped to `readme_presentation`) -- wraps the new `orchestrator.commit_generated_readme()` (extracted unchanged from `run_repo()`'s own inline git-add/commit logic), declares `idempotency_inputs`/`retry_policy` (`EFF-001`), and defines a real `reconciliation_check()` re-reading the work clone's embedded `resources`-span facts_hash; the `mode == "full"` gate lives inside this capability itself, not inherited from `supervisor/loop.py`'s planner-dispatch mode check (verified dead for this domain-scoped capability's real call path). `prepare_visual_asset.py` (Wave 7h, decision #41): the first capability to use `execution_type="manual_delivery_preparation"` (declared since the Wave 2 sprint, unused until now) -- validates an existing image asset (dimensions/format/size, via the new `Pillow` dependency) or prepares a real, freshly-generated candidate banner from product facts when none exists (confirmed live: zero existing image assets across the sampled registry pilots) -- never writes or embeds anything. `domains.py` (Wave 7+, decision #34): registered specialist-domain set (`KNOWN_DOMAINS`) backing `CapabilityManifest.allowed_domains` and `dispatcher.py`'s `caller_domain` check -- the dispatch-side enforcement boundary for specialist isolation, independent of whatever a composition framework offers (`CAP-006`). `effect_ledger.py` (Wave 5, decision #36, `EFF-002`/`EFF-003`; Wave 7a, decision #41): `dispatch_gated_effect()` -- a dispatch-tier two-phase pending/applied wrapper around `dispatch_tool_call()`, durable via `GitStateBackend`, plus `retry_is_safe()` for retry-inertness and a per-capability `reconciliation_check` hook (`registry.py::get_reconciliation_check()`) the ledger consults before treating a stale `pending` record as unrecoverable -- `EFF-001`, closed for real in Wave 7g against the real registered `commit_readme_write.py`, live-proven including a real crash-recovery reconciliation. `registry.all_tool_schemas(caller_domain=)` (Wave 7a): filters domain-scoped capabilities out of what an unscoped (general-planner) caller is offered |
| `state/` | Wave 4 (decision #32): `schema.py` (`RunStateV1`, `CapabilityOutputCacheEntry`), `backend.py` (`StateBackend` Protocol, `MEM-003`), `git_backend.py` (`GitStateBackend` -- one git ref per `org_repo` on this project's own remote, not a shared branch, so per-repository CAS (`MEM-002`) can't falsely conflict across unrelated repos; git plumbing only, no per-write working-tree checkout). Wave 8.5 (`SCL-005` extension): `acquire_run_lock()`/`release_run_lock()` -- a second, coarser, run-scoped lock on a genuinely different ref (`RUN_LOCK_REF_PREFIX`), closing the race where two concurrent `supervise_repo()` calls could both pay for the full specialist tier before either was rejected; shares its CAS/lease mechanics with the existing per-op lock via parameterized private helpers, not a duplicate implementation. Consulted additively by `orchestrator.generate_repo()` alongside the local work clone, opt-in via CLI `--durable-state`. `schema.py::DomainStateV1`/`RunStateV1.domain_states` and `domain_state.py::save_domain()` (Wave 6+, decision #35, `MEM-004`): per-specialist accepted state, additive alongside the flat fields above, composing the existing `acquire_lock`/`release_lock` lease as primary serialization with version-CAS as a lease-expiry backstop; `DomainStateV1.details: dict` (Wave 7a, decision #41) is a generic structured-payload escape hatch, matching `CapabilityOutputCacheEntry.result`'s plain-dict convention. `change_detection.py::classify_surface()` (Wave 7a, decision #41): the generic "did this specialist's tracked surface change" primitive extracted from `readme/reconciliation.py::classify()`'s general shape -- every Wave 7+ specialist's classify-first node calls this, not its own hand-rolled comparison |
