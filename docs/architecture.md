...
|---|---|
| `registry/` | `data/products.json` + `config/policies/*.yml` loading and the execution allow-list gate; `priority.py` validates `data/platform_priorities.json` and gives canonical portfolio runs Python -> .NET -> Java -> C++ -> TypeScript -> Rust -> Go order while retaining stable within-ecosystem registry order; `surface_ownership.py` materializes the five control classes and their allowed operations, permissions, and rollback; `discovery.py` owns the read-only GitHub client, naming hint, compatibility discovery view, and additive merge; `discovery_models.py` defines typed source/observation/inventory contracts; `discovery_inventory.py` retains every matched, ambiguous, and unmatched repository and explicit source failures; `self_heal.py` reconciles matched candidates before the public CLI execution allow-list gate and cannot report `NO_DRIFT` from an incomplete inventory (`CORE-034`, `L8-035`) |
