...
| `readme/document_operations.py` | Applies source-hash-bound operations in reverse byte order and gives independent zero-width insertions at one source boundary an explicit semantic order: header, overview/navigation, capabilities, installation, quick start, limitations, then license. This makes the visible journey independent of planner call order. |
