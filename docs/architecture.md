...
| `readme/` | Legacy gap/render compatibility remains in `gap_detector.py`, `renderer.py`, and `candidate_pipeline.py`; `document_plan.py`, `document_renderer.py`, and `document_validation.py` implement byte-preserving complete-document adoption, fact-cited section operations, reconstruction, and protected-content validation. |
