...
The reusable lifecycle is also the trusted cohort POC's production-path qualification surface.
Decision #85 freezes only source-current, checksum-valid `TRUSTED_NO_OP_PROVEN` candidates, then
decision #86 runs that exact cohort through an assurance-isolated `act_poc` profile, disposable
staging, a staging-only GitHub App installation, and authorized draft PRs before broad TRP-04
...
- `knowledge_evidence_verification.py` and `knowledge_source_polarity.py` independently recheck
  every selected claim's exact cited region against the immutable repository snapshot. Python uses
  its syntax tree; C#, Java, C++, TypeScript/JavaScript, Go, and Rust use the vendored tree-sitter
  parser stack. Implemented declarations, explicit stubs, mixed classes, missing citations, and
  synthetic pipeline artifacts remain distinct signals; file existence alone never verifies a
  claim.
- `knowledge_canonical_projection.py` conservatively projects only verified, conflict-free
  selected feature, directional-format, and limitation knowledge into a canonical product-truth
  field when no accepted repository fact already owns that field. It never overwrites accepted
  mechanical truth, never promotes unverified knowledge, strips internal assurance and source-path
  prose, and cites the exact selected knowledge fact through `supporting_fact_ids`. Resolution
  ranks factual assertions before blocked or missing collector observations, so an unavailable
  high-precedence evidence path cannot suppress a verified lower-precedence assertion.
- `readme/knowledge_claim_presentation.py` turns only verified, conflict-free feature,
  format-support, install, limitation, and troubleshooting items into bounded visitor content.
  Install metadata must exactly agree with verified acquisition; internal assurance prose and
  source locations never render. Capability, limitation, installation, and troubleshooting
  consumers carry the exact structured list-item coordinate into candidate provenance.
- `knowledge_application_evidence.py` builds provisional fact-stage and final post-render
  ...
  claim exact item-coordinate output spans or a typed omission/rejection reason. A fact-level span
  cannot promote an unrendered sibling. Acceptance fails closed on stale, provisional, internally
  inconsistent, or missing final accountability.
...
The separate `data/imported/presentation_knowledge.json` catalog preserves high-level capability
and limitation patterns discovered during development without granting Aspose.org factual or
runtime authority. `scripts/data-refresh/import_presentation_knowledge.py` freezes the exact
31-repository source tree and extracts only source-cited hints. At runtime,
`facts/presentation_knowledge.py` path-confines each citation and requires every technical anchor
to be rediscovered with the correct implementation/constraint polarity in the current immutable
product source. Only then may the hint fill a missing canonical fact; accepted mechanical truth is
never overwritten. The catalog, schema, and verifier are part of the fact-acceptance hash, and the
deployed system remains fully operable when the sibling checkout is absent.
...
|---|---|
| `registry/` | `data/products.json` + `config/policies/*.yml` loading and the execution allow-list gate; `naming.py` defines the fail-closed `Aspose[.-]{Family}-FOSS-for-{Platform}` eligibility contract; `priority.py` validates `data/platform_priorities.json` and gives canonical portfolio runs Python -> .NET -> Java -> C++ -> TypeScript -> Rust -> Go order while retaining stable within-ecosystem registry order; `surface_ownership.py` materializes the five control classes and their allowed operations, permissions, and rollback; `discovery.py` owns the read-only GitHub client and compatibility discovery view; `discovery_models.py` defines typed source/observation/inventory contracts; `discovery_inventory.py` retains every eligible, ambiguous, and excluded repository plus explicit source failure; `reconciliation.py` and `reconciliation_models.py` reconcile eligible entries by stable provider repository identity while preserving policy-owned fields and retaining conforming variants; `intake.py` deterministically classifies a source snapshot as preserve-first, ordinary full-pipeline, narrowly blocked, not applicable, or failed without granting approval or effect permission; `self_heal.py` applies the discovery ledger before the public CLI execution allow-list gate and cannot report `NO_DRIFT` from an incomplete inventory (`CORE-034`, `L8-035`, `L8-036`, `L8-037`) |
| `preflight/` | GitHub + LLM connectivity checks, fail-closed |
| `gitsafety/` | Clone, push-neuter, pre-push hook, independent verification; `baseline_reuse.py` reuses a persistent baseline only when its origin, exact probed revision, and empty worktree all match, otherwise falling back to the existing fresh-clone contract; `process.py` owns bounded subprocess-tree cancellation so a timed-out git operation cannot leave credential/git descendants holding pipes |
...
| `facts/aspose_knowledge_claims.py`; `facts/aspose_knowledge_selection.py`; `facts/knowledge_evidence_verification.py`; `facts/knowledge_source_polarity.py`; `facts/knowledge_canonical_projection.py`; `readme/knowledge_claim_presentation.py`; `facts/knowledge_application_evidence.py`; `facts/aspose_seo_keyword_facts.py` | Decision 106's native knowledge-application layer — load/index, freshness-gate, syntax-verify cited regions across supported ecosystems, bound-select, conservatively project verified claims into missing canonical truth, safely render, and item-coordinate evidence the imported aspose.org claim corpus and SEO keyword lists into `ProductFactsV2`; see "Native knowledge-application layer" above. |
| `facts/repository_examples.py`; `facts/verified_repository_examples.py`; `facts/platform_audience.py` | Ecosystem-neutral recovery of repository-maintained examples and conservative audience truth. README/source examples remain untrusted until the matching isolated verifier accepts them; audience text is derived only from an already-verified platform fact. |
| `facts/repository_format_extraction.py`; `facts/*_format_functionality.py` | Repository-native, family-scoped format-direction proof. Each corroborator binds an immutable clean revision and requires matching public declarations, implementations, and tests before emitting input/output evidence; a reader never implies output support and a writer never implies input support. The C++ Email adapter proves CFB, MSG, and EML independently of the development-only Aspose.org extractor. |
| `facts/presentation_knowledge_schema.py`; `facts/presentation_knowledge.py`; `scripts/data-refresh/import_presentation_knowledge.py`; `data/imported/presentation_knowledge.json` | Self-contained high-level presentation hints imported from the development benchmark and reverified against every current immutable product snapshot. Producer output is never runtime authority; path confinement, complete technical-anchor checks, directional source polarity, catalog hashing, and typed dispositions fail closed. |
| `facts/knowledge_qualification_models.py`; `facts/repository_knowledge_qualification.py`; `facts/portfolio_knowledge_qualification.py` | Offline zero-LLM, zero-effect knowledge qualification. It compiles current facts through the native template and blocking checks for diagnostic coverage only; it has no lifecycle, independent-review, Gate-A, or effect authority. |
| `facts/aspose_benchmark_profile.py`; `scripts/data-refresh/refresh_aspose_benchmark_profile.py` | Freezes the evolving Aspose.org `repo-presenter-regen-full` visitor-quality benchmark with stable complete inventories, runs its current audit battery only against an isolated copy, reconciles all 31 candidates, and emits a development-only quality profile. Benchmark bytes and verdicts never become facts or deployed runtime dependencies. |
...
| `ecosystems/registry_request.py`; `facts/acquisition*.py` | `registry_request.py` is the single coordinate-to-authoritative-request-URL seam used by both live resolvers and persisted-receipt validation. `acquisition_schema.py` defines checksum-complete authoritative-registry, isolated-source-build, and Python source-tree receipts and rejects a response URL that does not exactly match its coordinate; `acquisition_receipts.py` constructs those proofs without mixing receipt mechanics into path selection. `acquisition_pins.py` projects Python package source, TypeScript compiler/archive/built-artifact, and Rust lock/vendor/config checksums into one workload-pin contract. `acquisition.py` gives a published registry coordinate precedence and permits source acquisition only after receipt-backed registry absence plus successful pinned, network-denied proof. A Python revision whose declared build backend is invalid may expose an explicitly typed source-tree route only after its public imports and exact example execute from that immutable tree; it never renders a PyPI or `pip install` claim. Registry uncertainty, incomplete dependency inventories, and host-only builds remain blocked. The acceptance-contract hash includes this boundary so stale cached installation truth is recollected. |
| `presentation/` | `schema.py` defines `RepositoryPresentationPlanV1`; `markdown_structure.py`, `claim_validation.py`, `git_patch.py`, and `planner.py` provide deterministic structure, factuality, and native-Git patch proof; `document_planner.py` bridges fine-grained README operations into the repository surface plan. `template_schema.py` loads the assurance-neutral `RepositoryPresentationTemplateV1`, `template_adapters.py` binds either accepted inherited bytes or selected `ProductFactsV2` fields without promoting assurance, `template_compiler.py` assembles the canonical compact/standard/extended structure without generating product prose, and `visitor_contract.py` exposes the same visible invariants to the context-isolated blind reviewer without producer reasoning or factual conclusions. |
| `readme/` | Legacy gap/render compatibility remains in `gap_detector.py`, `renderer.py`, and `candidate_pipeline.py`. The complete-document pipeline is split by responsibility (no monoliths): `assessment.py` and `assessment_claims.py` inventory source-bound sections and material claims while treating prompt-like repository text as untrusted data; `claim_accountability_models.py` owns the typed accountability contracts, `claim_accountability.py` inventories every inherited/generated content unit as fact-bound, owner-required, uncertain, correction-required, unjustifiably lost, or unbound without treating preservation as approval, and `claim_accountability_validation.py` rechecks exact spans, complete inventories, correction operations, and approval blockers. The temporary trusted lane is separately typed: `trusted_composition_models.py` owns its plans and drafts, `trusted_composition_batching.py` enforces context/output envelopes and assigns presentation standards only to their structurally appropriate source batch, `trusted_composition_execution.py` performs one forced-tool batch call, `trusted_composition.py` orchestrates complete candidate and patch production, `trusted_composition_cache.py` persists content-addressed validated batches for retry idempotency, `trusted_composition_normalization.py` reconciles recoverable model schema/provenance/header defects to the bytes actually emitted, `trusted_composition_repair.py` replaces exactly one rejected hash-bound batch while preserving accepted batches, `trusted_composition_validation.py` binds every source/configuration ID, `trusted_composition_candidate_validation.py` enforces Markdown and presentation safety, `trusted_code_provenance.py` rejects executable candidate blocks that are not inherited from the source README, `trusted_candidate_terminology.py` owns visitor-only Enterprise Edition naming and promotional-link normalization without rewriting code fences, and `trusted_presentation_standards.py` derives generic policy/catalog-backed additions without repository-specific code. The repository-verified lane remains in `agentic_composition.py`, `agentic_composition_models.py`, `agentic_composition_grounding.py`, `agentic_composition_inputs.py`, and `agentic_composition_validation.py`; the agentic plan supplies a fact-ID-bound opening summary and input/capability/output vocabulary while retaining the exact hash-bound independent-review repair request, deterministic code owns Markdown and Mermaid syntax, `agentic_operation_coverage.py` fails closed when an actionable decision has no bounded edit, and `document_review_repairs.py` materializes only source-bound reviewer findings without discarding maintainer content. `acquisition_contracts.py` identifies contradicted package claims and exact stale coordinate-version spans; `document_plan.py` defines the typed byte-span plan and carries the complete claim-accountability map; `fact_grounding.py` locates literal selected-fact phrases and `claim_map.py` binds those phrases to exact candidate/source bytes; `document_structure.py` parses Markdown headings, sections, fenced-code source spans, and GitHub anchors; `document_templates.py` owns template loading/hashing and ecosystem-specific fact-to-section prose; `document_operations.py` applies hash-checked operations in reverse order; `document_hashing.py` owns shared SHA-256 helpers; `header_visual_models.py` defines the factual title, badge, and Mermaid contracts, `header_badges.py` selects the applicable package, platform, license, and contributor badges, `header_visual.py` renders sanitized repository-specific non-directional diagrams, `header_visual_validation.py` validates their grammar and provenance, and `document_header_visual.py` compiles them into bounded title/header/diagram operations while removing visitor-visible comments; `license_location.py` resolves accepted repository evidence to a safe relative license target and `document_legal.py` renders selected-license prose and practical permission benefits; `document_reconciliation.py` turns unresolved section assessments into exclusive, bounded withholding operations while retaining their exact source evidence; `document_link_hygiene.py` removes unbound Aspose targets without altering protected code, `document_links.py` materializes verified contextual bindings beside the exact accepted example, and `document_terminology.py` plans bounded Enterprise Edition corrections; `presentation_lint.py` composes facts-aware semantic and Markdown-structure rules into stable exact-span findings, with typed contracts and helpers split across the `presentation_lint_*` modules; `document_renderer.py` orchestrates candidate construction over the immutable `document_render_context.py`, while `document_opening.py`, `document_acquisition.py`, `document_examples.py`, `document_limitations.py`, `document_legal.py`, and `document_release.py` independently select bounded operations for their respective sections; and `document_validation.py` independently reconstructs the marker-free candidate, validates protected content, rejects an absent, stale, or incomplete accountability map, and runs the complete deterministic presentation lint before independent agent review. Legacy complete-document markers remain readable only for migration; production provenance lives in the durable document plan and evidence. The pre-extraction contracts, before/after map, and frozen hashes are recorded in `docs/readme-composition-seams.md`. |
| `readme/presentation_contract.py` | Owns the assurance-neutral visible README contract constants consumed by both trusted and verified lanes: common header/section journey, no-emoji policy, and detailed Inputs/formats → Product/API → Capabilities → Outputs/formats Mermaid bounds. |
| `readme/code_fence_presentation.py` | Owns public code-fence language and whitespace normalization plus independent inspection, preventing unhighlighted fences and repeated blank code lines without changing executable content. |
| `readme/header_visual_layout.py`, `readme/header_visual_mermaid.py`, `readme/header_visual_validation.py` | Own the fixed-grid Mermaid block layout, common-width endpoint groups, source rendering, and independent structural validation: one compact vertical capability column through five entries and two balanced columns above five. |
| `verification/mermaid_render.py`, `specialists/readme_visual_render_gate.py` | Auto-provision and run a pinned official Mermaid CLI, parse the resulting SVG, and fail deterministic promotion unless the actual render has the expected semantic nodes/connectors, compact landscape geometry, non-overlapping adaptive Core columns, and uniform peer endpoint widths. Passing proofs are cached only by renderer contract and exact Mermaid-source hash. |
| `readme/document_navigation.py` | Finalizes the Navigation body inside its existing bounded owner operation after every planned section operation is known, so the rendered list covers the actual final H2 set without a whole-document rewrite or overlapping edit. |
| `readme/document_section_order.py` | Enforces the accepted repository-presentation journey after source-bound composition by moving complete applicable H2 blocks in a candidate-coordinate operation, preserving each section body and unknown section while synchronizing Navigation. |
| `readme/document_section_journey.py` | Consolidates inherited Why/Features variants into one open `Key capabilities` section after the overview. Its claim inventory is derived from accepted, fact-cited visual/fact nodes; useful non-list source context remains visible, while unverified inherited lists cannot displace the accepted capability set. |
| `readme/document_core_opening.py` | Plans overview/navigation, missing Installation, and missing Quick start as separate fact-bound operations at a shared source boundary so semantic ordering remains explicit without expanding the renderer orchestrator. |
| `readme/document_operations.py` | Applies source-hash-bound operations in reverse byte order, then applies non-overlapping candidate-coordinate exact section moves; independent zero-width insertions at one source boundary retain an explicit semantic order: header, overview/navigation, capabilities, installation, quick start, limitations, then license. This makes the visible journey independent of planner call order and source section order. |
| `readme/trusted_portfolio_brand.py` | Adapts the shared presentation contract to README-inherited configured standards, normalizes trusted candidates, validates contextual Enterprise Edition placement, and performs side-by-side cohort consistency checks. |
| `specialists/review_role_execution.py` | Treats reviewer output as an untrusted proposal. `review_candidate_anchors.py` supplies content-addressed Markdown block identities so blind reviewers select stable candidate evidence instead of transcribing approximate quotes; `review_mechanical_observations.py` supplies typed parser-owned counts and compliance results that the reviewer may cite but not reinterpret; `review_standard_premises.py` rejects claims that invert configured collapsible-example, Mermaid-role/connector, or Enterprise-scope rules; role execution resolves those identities and observations back to exact bytes and values. Legacy whitespace-lossy quote binding remains fail-closed and only succeeds when its content-normalized match is unique; ambiguous, absent, or contract-contradicted premises fail closed. |
| `supervisor/local_poc_cache.py`, `supervisor/local_poc_acceptance_binding.py`, `supervisor/local_poc_manifest_recovery.py`, `supervisor/local_poc_noop_reuse.py` | Bind approved and completed revision bundles to source, facts, templates, prompt scopes, reviewer standard, control-plane fingerprint, and checksum inventory. The acceptance-binding module chains deterministic validation, independent review, final verdict, and no-op proof to the same candidate and candidate-stage dependency identity, so a partially refreshed compatibility manifest cannot launder stale review evidence. A compatibility manifest left behind by an older reducer is repaired only when its accepted candidate-stage receipt, exact artifacts, and durable lifecycle hashes independently agree. The first unchanged rerun then promotes an `AGENT_APPROVED` bundle to `NO_OP_PROVEN` before clone, author, or reviewer dispatch and records an exact zero-provider-call cache receipt; later runs use the completed-cache shortcut. |
...
| `readme/document_overview.py`, `readme/presentation_similarity.py` | Fact-backed overview rendering and one shared semantic-overlap contract, so the generator and presentation lint cannot disagree about duplicated problem/capability inventories. |
| `readme/capability_semantics.py`, `readme/limitation_semantics.py` | Domain-aware public-claim normalization that preserves distinct verified capability domains and recognizes only assurance-equivalent limitation paraphrases. The same limitation equivalence contract drives source-detail reconciliation and deterministic duplicate linting. |
| `presentation/verified_source_limitation_matching.py` | Suppresses inherited limitation wording only when exactly one canonical Scope and Limitations row carries the selected limitation fact and its exact structured coordinate; unrelated constraints remain preserved. |
| `readme/document_presentation_repairs.py` | Bounded deterministic corrections for exact visitor-facing defects that are safe to repair without model judgment: standalone commercial call-to-action directories, unverified future placeholders, unexplained option tokens, and known punctuation mojibake. |
| `validation/` | The legacy 10-rule deterministic registry plus `presentation_template.py`, which enforces the reusable README structure, bound badge row, list navigation, detailed non-directional product Mermaid, no-comments/no-emoji policy, cross-product identity isolation, license prose, notices, and copyright default. |
| `validation/public_candidate_quality.py`; `validation/public_quality_*.py` | Canonical public-candidate quality gate family. The facade preserves the stable API; responsibility-sized modules own typed findings, grounding and contradiction checks, visitor-facing lint, structural checks, and the ordered registry. The versioned source hash covers the complete ordered family so cache and evidence reuse fail closed when any check changes. |
...
| `github_api/` | Wave 7b (decision #41): `client.py` -- shared, read-only GitHub REST API client (GET-only), extending `scripts/update_products_registry.py`'s live-proven pagination/rate-limit pattern rather than a fourth/fifth independent reimplementation. `repo_summary`/`get_branch`/revision-addressed `get_file_content` support immutable staging reconciliation; `list_contributors`/`list_languages` (7b), `list_releases` (7c), `get_community_profile` (7e -- live-verified its `files` object reports recognition for exactly `readme`/`license`/`code_of_conduct`/`contributing`/`issue_template`/`pull_request_template`, never `security`/`support`). `write_client.py` keeps draft-PR creation explicit and separate from all reads. |
| `capabilities/` | Wave 2 onward: typed registry/dispatcher plus one module per capability. `contracts.py` materializes strict Pydantic input and output models from every registered manifest; dispatch rejects malformed arguments, undeclared wiring arguments, missing permissions, and executor output that violates its contract. `compatibility.py` owns the ecosystem-to-build/package/registry vocabulary, validates manifest declarations at registry construction, and supports repository-profile filtering. Registry construction also rejects incomplete permission declarations, unknown validators, invalid evidence fields, and mutating capabilities without evidence/idempotency/retry contracts. Read-only capabilities inspect/profile/audit/propose; `extract_trusted_readme_facts.py` is the assurance-specific local-only inventory seam and `compose_trusted_readme.py` is the forced-tool, context-bounded trusted authoring seam, while `get_product_facts.py` retains repository-verified collection. `plan_readme_composition.py` is the domain-scoped agentic authoring proposal over one immutable snapshot and verified fact graph, while `build_presentation_plan.py` independently re-derives facts, bounded document operations, claim spans, and ownership before verification. `commit_readme_write.py` is the `readme_presentation` domain's `local_write` effector and `open_presentation_pr.py` (decision #51) is its first `remote_write` effector. Both declare idempotency/reconciliation/evidence contracts, require `mode: full`, and run through `effect_ledger.dispatch_gated_effect()`. `staging_target.py` binds source repositories to explicitly disposable, stable-ID-verified targets; `staging_effect.py` converts a frozen qualified trusted member into one immutable proposal; and `presentation_pr_effect.py` owns the stable branch, exact-byte commit, draft-PR create/update, drift rejection, and lost-response reconciliation mechanics. Applied ledger entries with reconciliation hooks are re-observed: intact effects remain no-ops, while missing idempotent effects are safely reapplied. The proposal contracts live in `state/proposal_schema.py` as `TrustedTransformationProposalV1` and `OpenProposalV2`. `prepare_visual_asset.py` remains manual-delivery preparation only. `domains.py` and `registry.all_tool_schemas(caller_domain=)` enforce specialist-domain isolation independently of orchestration-framework tool scoping. |
| `state/` | Durable Git-ref CAS state and leases. `schema.py` keeps V1 as an explicit migration source and makes `RunStateV2` current; `migrations.py` fails closed on unknown versions. `git_backend.py` bulk-loads exact remote ref names into numeric isolated refs so case-distinct historical names cannot collapse on Windows. `lifecycle_schema.py` defines trigger/checkpoint/health plus additive README-intake contracts; `readme_poc_intake.py` reserves, resumes, deduplicates, and completes one logical read-only intake per source/classification contract without regressing an already advanced README stage; `mission_goal_schema.py` defines governed goal, contribution, portfolio-scoreboard, exact-next-task, and closeout-evidence records; `lifecycle.py` coordinates acceptance, transitions, resume, and active context; `cas.py`, `checkpoints.py`, and `recovery.py` own their named persistence responsibilities; `trigger_v2.py` normalizes provider identity; `health.py` aggregates portfolio health. Existing domain, freshness, effect, and model-route records remain additive within the same per-repository state. |
...
| `specialists/` | `registry.py` owns registry-driven domain ordering, dependency checks, and complete domain registration. `readme_reconciliation.py`, `github_generated_surface_audit.py`, `package_release_audit.py`, `metadata_presentation.py`, `community_files_presentation.py`, `cross_surface_validation.py`, `readme_presentation.py`, `visual_preparation.py`, `presentation_benchmarking.py`, and `independent_verification.py` run through typed capability dispatch. `metadata_presentation.py` dispatches only `propose_metadata_changes(org_repo)`; that capability independently derives facts through `facts/provider.py`. `readme_presentation.py` alone reaches the local README effect, only after its candidate has a structured, executable presentation plan and passes factuality and independent-verification gates. `readme_presentation_review.py` owns the separate gate node, `readme_review_validation.py` owns deterministic candidate/bundle dispatch and reconstruction, and `readme_review_repair.py` owns reviewer-instructed regeneration plus full revalidation; together they propagate only the final accepted candidate. `readme_review_roles.py` defines hash-bound, context-isolated blind-quality and factual-plan contracts; `factual_review_packet.py` projects the full facts/plan artifacts into selected evidence, source dispositions, operation summaries, and exact candidate claim anchors so the reviewer never receives unrelated producer bulk. The assurance-specific trusted lane uses `trusted_transform_review_models.py`, `trusted_fidelity_context.py`, `trusted_fidelity_cache.py`, `trusted_fidelity_execution.py`, `trusted_fidelity_validation.py`, and `trusted_transform_review.py` to compact, checkpoint, and batch source-complete evidence before deterministically reducing separate blind-quality and inheritance-fidelity roles without granting factual authority; `trusted_transform_review_repair.py` permits two section-scoped changed-byte repairs and reruns both roles. `review_finding_grounding.py` requires every favorable or unfavorable conclusion to cite an exact candidate span; factual conclusions additionally cite the selected fact, exact authoritative source location and excerpt, and checked directional polarity. Literal selected facts cannot be reclassified as missing, and each immutable role record persists the complete validation result covering every finding. `review_role_execution.py` owns bounded semantic correction of invalid role outputs, while `readme_review_reducer.py` rejects author/reviewer identity overlap and derives lifecycle and repair authority only from validated findings. `separated_readme_review.py` invokes both governed routes. `readme_repair_contracts.py` defines the immutable repair receipt; `readme_repair_validation.py` binds it to finding IDs, rejected spans, changed source operations, before/after hashes, and inspectable deltas. `readme_review_repair_loop.py` permits rereview only after that receipt is valid, and `readme_review_repair_state.py` durably reopens unchanged or irrelevant repairs as agent-fixable composition work. The historical single `independent_readme_review.py` route remains available to diagnostic/golden-set callers but the presentation review node no longer invokes it as acceptance authority. |
| `specialists/bounded_review_packets.py`; `specialists/bounded_review_*.py` | Oversized independent-review packetization and recovery. The facade/planner deterministically partitions candidate structure and accountable claims; separate modules own contracts, hashing, structure, accountability, packing, coverage, results, and bounded repairs. `bounded_review_execution.py` alone invokes the registered reviewer roles. Canonical typed contracts fail closed on invalid packetization rather than silently falling back to a less accountable merged review. |
...
| `supervisor/` | The canonical production runtime. `intake.py` performs the local-POC read-only source probe through the existing baseline clone, writes compact collision-checked receipts, and hands only ready routes to the normal preflight/runtime; `intake_cache.py` skips a repeated intake only after stable identity, source revision, classification contract, receipt payload hash, and both no-effect flags all revalidate; `loop.py` owns repository lifecycle/locks; `stage_limit.py` defines typed local proof ceilings so a bounded campaign cannot cross into a later agentic or effect stage; `execution_profile.py` defines both the local POC and the Actions-only, local-effect-only `act_poc` assurance boundary; `trusted_cohort_runtime.py` admits only exact current members of a frozen trusted cohort and emits its authoritative Actions matrix; `trusted_cohort_restore.py` checksum-validates and idempotently restores the cohort's revision-addressed runtime roots and isolated state bundle for live-like workflow proof. `portfolio_scheduler/` owns stable campaign/work/fence contracts, compact private-attempt storage, seal validation, and the sole serialized reducer that promotes compatibility artifacts, receipts, and lifecycle state; a newly accepted stage receipt replaces stale stage-owned manifest fields, while an exact receipt retry preserves later lifecycle evidence. `product_truth.py` prepares, caches, persists, and invalidates one revision- and prompt-bound repository-verified `ProductFactsV2` graph. `trusted_product_truth.py` provides the disjoint README-inherited preparation/persistence seam without calling those verification providers; `trusted_readme_pipeline.py` orchestrates the trusted candidate/review/no-op stages, with typed results in `trusted_readme_pipeline_models.py`, bounded dispatch in `trusted_readme_stage_execution.py`, assurance-isolated artifacts in `trusted_readme_evidence.py`, and exact unchanged proof in `trusted_readme_noop.py`; `trusted_review_state.py` records validation and independent trusted-review outcomes and makes an identical accepted retry a durable no-op. `trusted_cohort.py` derives and freezes the production-path POC cohort, `trusted_cohort_qualification.py` owns its per-member lifecycle, current-contract, checksum, review, no-op, and target-freshness admission boundary, and `trusted_cohort_reconstruction.py` independently repeats membership reduction from fresh state and target observations. `portfolio_stage_cache.py` validates revision-addressed, checksum-complete bounded-stage bundles and their reducer receipts so portfolio slices advance without repeating accepted work; `local_poc_cache.py` makes completed-bundle reuse a typed, fail-closed decision bound to the live revision and every material fact, prompt, template, validator, reviewer, control-plane, and artifact fingerprint, and identifies the earliest affected lifecycle stage on a miss; `local_poc_manifest_recovery.py` repairs only receipt-proven stale compatibility identities before approved no-op reuse; `execution_context.py` carries run-scoped proposal-only, fact-graph, and stage-limit boundaries without adding a second controller; `specialist_tier.py` owns isolated domain execution and retry; `planner_loop.py` owns bounded capability selection; `action_dispatch.py` owns permission/effect/repair dispatch; `finding_status.py` classifies specialist findings/proposals and `work_ledger.py` deterministically prevents them from collapsing into false no-change or premature planner stop results; `local_poc_evidence.py` materializes snapshot/fact/candidate boundaries and keeps trusted facts under an assurance-specific subtree so they cannot replace verified evidence; `local_poc_review_evidence.py` writes deterministic review, independent verdict, repair history, final verdict, and unchanged-rerun proof into that same revision-addressed bundle. `state_tracking.py`, `evidence.py`, `models.py`, and `status.py` own their named contracts. `task.py` owns repository tasks; `mission_schema.py`, `mission_graph.py`, `mission_control.py`, and `mission_command.py` own the central mission task graph, while `mission_goal_guard.py` derives the dynamic durable lifecycle scoreboard and rejects contribution-free closure. `readme-agent supervise` accepts exactly one target: `--repo`, `--registry`, or `--mission-task-graph`; all remain one supervisor authority. |
| `supervisor/portfolio_proof_engine/repository_worker_*.py`; `supervisor/portfolio_worker_*.py` | Bounded repository-worker acceleration under the canonical supervisor. Typed contracts bind each child invocation to registry revision, repository, immutable source revision, and a unique invocation ID; process isolation and resource classes constrain concurrency; the scheduler owns admission and aggregation. Only a successful current child may contribute its matching receipt, so stale success evidence cannot mask a failed retry. |
