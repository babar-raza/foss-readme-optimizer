...
| `readme/` | Legacy gap/render compatibility remains in `gap_detector.py`, `renderer.py`, and `candidate_pipeline.py`. The complete-document pipeline is split by responsibility (no monoliths): `assessment.py` and `assessment_claims.py` inventory source-bound sections and material claims while treating prompt-like repository text as untrusted data; `agentic_composition.py` validates the separate authoring planner's source/fact/assessment-bound section decisions and grounded overview sentences; `acquisition_contracts.py` identifies only contradicted package-claim spans; `document_plan.py` defines the typed byte-span plan; `claim_map.py` binds every introduced or corrected claim to exact candidate/source bytes and an accepted selected fact; `document_structure.py` parses Markdown headings/sections and GitHub anchors; `document_templates.py` owns template loading/hashing and ecosystem-specific fact-to-section prose; `document_operations.py` applies hash-checked operations in reverse order; `document_hashing.py` owns shared SHA-256 helpers; `document_renderer.py` selects bounded editorial operations; and `document_validation.py` independently reconstructs the candidate and validates protected content. |
