...
| `supervisor/local_poc_cache.py`, `supervisor/local_poc_noop_reuse.py` | Bind approved and completed revision bundles to source, facts, templates, prompt scopes, reviewer standard, control-plane fingerprint, and checksum inventory. The first unchanged rerun promotes an `AGENT_APPROVED` bundle to `NO_OP_PROVEN` before clone, author, or reviewer dispatch and records an exact zero-provider-call cache receipt; later runs use the completed-cache shortcut. |
