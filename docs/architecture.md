...
| `facts/` | `schema.py` retains the narrow V1 compatibility contract. `schema_v2.py` defines the provenance-complete required-field inventory; `migration.py` explicitly converts V1 without inventing missing values; `provider.py` re-derives policy/repository facts for both the facts and metadata capabilities without capability-to-capability calls; `resolution.py` applies source precedence and records conflicts; `gating.py` maps facts to dependent surfaces, rejects non-selected citations, and validates technical claims; `protected_content.py` fingerprints commands, examples, terminology, limitations, and maintainer regions through `markdown-it-py`; `example_execution.py` owns the bounded, secret-free local execution boundary (not an OS sandbox). |
| `presentation/` | `schema.py` defines `RepositoryPresentationPlanV1` and enforces finding/action/source-span, ownership-operation, citation-union, path, and hash integrity; `markdown_structure.py` uses `markdown-it-py` for heading, code, link, image, and UTF-8 byte-span structure; `claim_validation.py` compares the actual license, URLs, and relationship prose inside the owned span with selected fact values; `git_patch.py` applies hash-guarded non-overlapping byte edits and delegates patch generation/acceptance to native `git diff` and `git apply --check`; `planner.py` assesses all ten presentation dimensions and permits only revision-bound, ownership-compatible, semantically fact-backed actions. |
