...
| `supervisor/` | Wave 5 (decision #36, `AGT-001`/`003`/`004`, `ORC-001`): the production planning loop. `task.py` owns product-repository capability tasks; `mission_schema.py` owns the strict V1 central-taskcard contracts, `mission_graph.py` owns escape-safe YAML loading plus dependency/cycle validation, and `mission_control.py` owns deterministic selection, CAS-backed claims/transitions, and mission-aware stop evaluation; `mission_command.py` is the thin CLI adapter. `convergence.py` owns repository-run convergence, `repair.py` owns scoped repair creation, and `loop.py` wires `supervise_repo()` through the registered specialist/capability system. `readme-agent supervise` accepts exactly one target: `--repo` for product supervision or `--mission-task-graph` for central mission control; both remain one supervisor authority, not competing controllers. |
