...
| `commands.py`, `commands_*.py` | Stable CLI façade plus responsibility-sized compatibility, canonical-supervision, and governance handlers. Legacy `generate`/`run`/`run-registry` invoke `supervise_repo()` with read-only effect permissions; they cannot commit or push. |
