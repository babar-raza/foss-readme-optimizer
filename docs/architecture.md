...
| `supervisor/local_poc_cache.py`, `supervisor/local_poc_manifest_recovery.py`, `supervisor/local_poc_noop_reuse.py` | Bind approved and completed revision bundles to source, facts, templates, prompt scopes, reviewer standard, control-plane fingerprint, and checksum inventory. A compatibility manifest left behind by an older reducer is repaired only when its accepted candidate-stage receipt, exact artifacts, and durable lifecycle hashes independently agree. The first unchanged rerun then promotes an `AGENT_APPROVED` bundle to `NO_OP_PROVEN` before clone, author, or reviewer dispatch and records an exact zero-provider-call cache receipt; later runs use the completed-cache shortcut. |
